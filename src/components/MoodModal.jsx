import React from 'react';
import { X, Sparkles, Heart } from 'lucide-react';

/**
 * MoodModal - Interactive Mood Management Modal
 *
 * Displays mood-based wellness recommendations
 * Users can click suggestions to add them to tasks or food diary
 *
 * Features:
 * - Mood-specific wellness activities
 * - Mood-boosting nutrition suggestions
 * - Click-to-add functionality
 * - Supplement recommendations
 * - Tap-away or X button to close
 */
const MoodModal = ({
  isOpen,
  onClose,
  currentMood,
  moodWellness,
  darkMode,
  onAddTask,
  onAddToFoodDiary,
  currentHour,
  selectedActions = [],
  onActionSelect
}) => {
  if (!isOpen || !moodWellness) return null;

  const handleAddActivity = (activity) => {
    onAddTask(currentHour, activity);
    if (onActionSelect) {
      onActionSelect(activity);
    }
  };

  const handleAddFood = (food) => {
    onAddToFoodDiary(food);
    if (onActionSelect) {
      onActionSelect(food);
    }
  };

  const isActionSelected = (action) => {
    return selectedActions.includes(action);
  };

  const color = moodWellness.color || 'purple';

  const getColorClasses = () => {
    const colors = {
      rose: {
        bg: darkMode ? 'bg-rose-500/10' : 'bg-rose-50',
        border: darkMode ? 'border-rose-500/30' : 'border-rose-200',
        text: darkMode ? 'text-rose-400' : 'text-rose-600',
        button: darkMode ? 'bg-rose-500/20 hover:bg-rose-500/30 border-rose-500/40' : 'bg-rose-100 hover:bg-rose-200 border-rose-300',
        gradient: darkMode ? 'from-rose-900/50 via-pink-900/50 to-rose-900/50' : 'from-rose-100 to-pink-100'
      },
      purple: {
        bg: darkMode ? 'bg-purple-500/10' : 'bg-purple-50',
        border: darkMode ? 'border-purple-500/30' : 'border-purple-200',
        text: darkMode ? 'text-purple-400' : 'text-purple-600',
        button: darkMode ? 'bg-purple-500/20 hover:bg-purple-500/30 border-purple-500/40' : 'bg-purple-100 hover:bg-purple-200 border-purple-300',
        gradient: darkMode ? 'from-purple-900/50 via-violet-900/50 to-purple-900/50' : 'from-purple-100 to-violet-100'
      },
      blue: {
        bg: darkMode ? 'bg-blue-500/10' : 'bg-blue-50',
        border: darkMode ? 'border-blue-500/30' : 'border-blue-200',
        text: darkMode ? 'text-blue-400' : 'text-blue-600',
        button: darkMode ? 'bg-blue-500/20 hover:bg-blue-500/30 border-blue-500/40' : 'bg-blue-100 hover:bg-blue-200 border-blue-300',
        gradient: darkMode ? 'from-blue-900/50 via-indigo-900/50 to-blue-900/50' : 'from-blue-100 to-indigo-100'
      },
      orange: {
        bg: darkMode ? 'bg-orange-500/10' : 'bg-orange-50',
        border: darkMode ? 'border-orange-500/30' : 'border-orange-200',
        text: darkMode ? 'text-orange-400' : 'text-orange-600',
        button: darkMode ? 'bg-orange-500/20 hover:bg-orange-500/30 border-orange-500/40' : 'bg-orange-100 hover:bg-orange-200 border-orange-300',
        gradient: darkMode ? 'from-orange-900/50 via-amber-900/50 to-orange-900/50' : 'from-orange-100 to-amber-100'
      }
    };
    return colors[color] || colors.purple;
  };

  const colorClasses = getColorClasses();

  // Mood emoji mapping
  const moodEmojis = {
    energized: '⚡',
    focused: '🎯',
    calm: '😌',
    tired: '😴',
    anxious: '😰',
    overwhelmed: '😵'
  };

  const moodEmoji = moodEmojis[currentMood] || '😊';

  return (
    <>
      {/* Backdrop */}
      <div
        className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 transition-opacity"
        onClick={onClose}
      />

      {/* Modal */}
      <div className="fixed inset-0 z-50 overflow-y-auto pointer-events-none">
        <div className="min-h-screen flex items-center justify-center p-4 py-12">
          <div
            className={`max-w-lg w-full pointer-events-auto rounded-2xl shadow-2xl ${
              darkMode ? 'bg-gray-900 border-2 border-gray-700' : 'bg-white border-2 border-gray-200'
            }`}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className={`flex items-center justify-between p-6 border-b ${
              darkMode ? 'border-gray-800' : 'border-gray-200'
            }`}>
            <div className="flex items-center space-x-3">
              <Heart className={colorClasses.text} size={28} />
              <div>
                <h2 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                  Mood: {currentMood} {moodEmoji}
                </h2>
                <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  {moodWellness.title}
                </p>
              </div>
            </div>
            <button
              onClick={onClose}
              className={`p-2 rounded-lg transition-all ${
                darkMode
                  ? 'hover:bg-gray-800 text-gray-400 hover:text-gray-200'
                  : 'hover:bg-gray-100 text-gray-600 hover:text-gray-900'
              }`}
            >
              <X size={24} />
            </button>
          </div>

          {/* Content */}
          <div className="p-6 space-y-6">
            {/* Message */}
            <div className={`p-4 rounded-lg bg-gradient-to-br ${colorClasses.gradient} border-2 ${colorClasses.border}`}>
              <p className={`text-sm ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>
                {moodWellness.message}
              </p>
            </div>

            {/* Wellness Activities */}
            {moodWellness.activities && moodWellness.activities.length > 0 && (
              <div>
                <h3 className={`text-sm font-bold uppercase tracking-wide mb-3 ${
                  darkMode ? 'text-gray-400' : 'text-gray-600'
                }`}>
                  <Sparkles className="inline mr-1" size={14} />
                  Wellness Activities (Click to Add to Tasks)
                </h3>
                <div className="space-y-2">
                  {moodWellness.activities.map((activity, idx) => {
                    const isSelected = isActionSelected(activity);
                    return (
                      <button
                        key={idx}
                        onClick={() => handleAddActivity(activity)}
                        className={`w-full text-left p-3 rounded-lg border-2 transition-all ${
                          isSelected
                            ? darkMode
                              ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-300 line-through opacity-60'
                              : 'bg-emerald-100 border-emerald-400 text-emerald-700 line-through opacity-60'
                            : colorClasses.button + ' ' + (darkMode ? 'text-gray-200' : 'text-gray-800')
                        }`}
                        disabled={isSelected}
                      >
                        <span className="text-sm">{isSelected ? '✓✓' : '✨'} {activity}</span>
                      </button>
                    );
                  })}
                </div>
              </div>
            )}

            {/* Mood-Boosting Nutrition */}
            {moodWellness.snacks && moodWellness.snacks.length > 0 && (
              <div>
                <h3 className={`text-sm font-bold uppercase tracking-wide mb-3 ${
                  darkMode ? 'text-gray-400' : 'text-gray-600'
                }`}>
                  🍽️ Mood-Boosting Nutrition (Click to Add to Food Diary)
                </h3>
                <div className="grid grid-cols-1 gap-2">
                  {moodWellness.snacks.map((snack, idx) => {
                    const isSelected = isActionSelected(snack);
                    return (
                      <button
                        key={idx}
                        onClick={() => handleAddFood(snack)}
                        className={`text-left p-3 rounded-lg border transition-all ${
                          isSelected
                            ? darkMode
                              ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-300 line-through opacity-60'
                              : 'bg-emerald-100 border-emerald-400 text-emerald-700 line-through opacity-60'
                            : darkMode
                              ? 'bg-white/5 hover:bg-white/10 border-white/10 hover:border-white/20 text-gray-300'
                              : 'bg-gray-50 hover:bg-gray-100 border-gray-200 hover:border-gray-300 text-gray-700'
                        }`}
                        disabled={isSelected}
                      >
                        <span className="text-sm">{isSelected ? '✓✓' : '🍽️'} {snack}</span>
                      </button>
                    );
                  })}
                </div>
              </div>
            )}

            {/* Supplement Recommendation */}
            {moodWellness.supplement && (
              <div className={`p-4 rounded-lg ${colorClasses.bg} border ${colorClasses.border}`}>
                <h3 className={`text-xs font-bold uppercase tracking-wide mb-2 ${
                  darkMode ? 'text-gray-400' : 'text-gray-600'
                }`}>
                  💊 Supplement Recommendation
                </h3>
                <p className={`text-sm ${colorClasses.text}`}>
                  {moodWellness.supplement}
                </p>
              </div>
            )}
          </div>

            {/* Footer */}
            <div className={`p-4 border-t ${
              darkMode ? 'border-gray-800' : 'border-gray-200'
            }`}>
              <button
                onClick={onClose}
                className={`w-full py-3 rounded-lg font-medium transition-all ${
                  darkMode
                    ? 'bg-gray-800 hover:bg-gray-700 text-gray-200'
                    : 'bg-gray-100 hover:bg-gray-200 text-gray-900'
                }`}
              >
                Close
              </button>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default MoodModal;
